...
export default function ManagerTodayDeliveryPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setOrders(allOrders.filter(o => o.orderStatus === "DELIVERED" || new Date(o.actualDelivery) >= today));
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-base sm:text-lg md:text-xl font-semibold text-[#00215B]">{t.todayDelivery}</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
        {[
          ...
          { label: t.todayOrders, value: stats?.todayOrders || 0, icon: Truck, color: "text-[#7C3AED] bg-[#F3E8FF]" },
          { label: t.pendingToday, value: stats?.todayPendingOrders || 0, icon: Truck, color: "text-[#D4A017] bg-[#FFF8E1]" },
          { label: t.todaySales, value: `৳${(stats?.todaySales || 0).toLocaleString()}`, icon: Truck, color: "text-[#EC008C] bg-[#FCE8F3]" },
        ].map((s) => (
