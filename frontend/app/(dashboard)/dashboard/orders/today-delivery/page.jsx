...
import { Search, Eye, Printer, CheckCircle, DollarSign, ShoppingCart, TrendingUp, ArrowLeft } from "lucide-react";
...
export default function TodayDeliveryPage() {
  ...
  const fetchData = async () => {
    try {
      const [statsRes, ordersRes] = await Promise.all([
        ...
        api.get("/orders/admin/all?status=DELIVERED"),
      ]);
      setStats(statsRes.data.data);
      const allOrders = ordersRes.data.data || [];
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      setOrders(allOrders.filter(o => new Date(o.actualDelivery) >= today));
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-4">{t.todayDelivery}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-green-50 p-2.5 rounded-lg"><CheckCircle size={20} className="text-green-600" /></div>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800 mt-1">৳{(stats?.totalRevenue || 0).toLocaleString()}</p>
            </div>
            <div className="bg-indigo-50 p-2.5 rounded-lg"><TrendingUp size={20} className="text-indigo-600" /></div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                paginated.map((order) => (
                  <tr key={order.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">{formatDate(order.actualDelivery)}</td>
                    <td className="px-4 py-3">
