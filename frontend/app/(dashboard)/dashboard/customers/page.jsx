...
import { Users, Ban, CheckCircle, Edit2, Trash2, X, Shield, UserCog } from "lucide-react";
                ...
                <tr key={user.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => handleEdit(user)} className="text-blue-500 hover:text-blue-700 p-1 rounded hover:bg-blue-50 transition" title="Edit user">
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => toggleBlock(user.id, user.isBlocked)} className={`p-1 rounded transition ${user.isBlocked ? "text-green-500 hover:text-green-700 hover:bg-green-50" : "text-red-500 hover:text-red-700 hover:bg-red-50"}`} title={user.isBlocked ? "Unblock" : "Block"}>
                        {user.isBlocked ? <CheckCircle size={14} /> : <Ban size={14} />}
                      </button>
                      {user.role !== "ADMIN" && (
                        <button onClick={() => handleDelete(user.id)} className="text-red-500 hover:text-red-700 p-1 rounded hover:bg-red-50 transition" title="Delete user">
                          <Trash2 size={14} />
                        </button>
                      )}
                    </div>
