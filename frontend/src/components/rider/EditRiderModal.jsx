...
export default function EditRiderModal({ rider, onClose, onUpdated }) {
  const [form, setForm] = useState({
    ...
    assignedZila: rider.assignedZila || "",
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.put(`/riders/${rider.id}`, form);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto z-10">
        <div className="sticky top-0 bg-white border-b border-[#E5E7EB] px-5 py-3 flex items-center justify-between rounded-t-xl">
          <h2 className="text-sm font-semibold text-[#000000]">Edit Rider</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[#364152] mb-1">Vehicle Type</label>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[#364152] mb-1">Vehicle Number</label>
              <input type="text" value={form.vehicleNumber} onChange={(e) => setForm({ ...form, vehicleNumber: e.target.value })}
                className="w-full border border-[#E5E7EB] rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#E5E7EB]" />
            </div>
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-[#364152] mb-1">License Number</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-[11px] text-[#364152] font-medium">Available</span>
          </label>
